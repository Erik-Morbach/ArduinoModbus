...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    (void)ctx;
    RS485Class *_serial = (RS485Class *) ctx->serialType;

    ssize_t size;
    _serial->noReceive();
    _serial->beginTransmission();
    size = _serial->write(req, req_length);
...
static ssize_t _modbus_rtu_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
    ...
    (void)ctx;
    RS485Class *_serial = (RS485Class *) ctx->serialType;
    return _serial->readBytes(rsp, rsp_length);
#else
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    if (!SetCommState(ctx_rtu->w_ser.fd, &dcb)) {
        ...
        _serial->begin(ctx_rtu->baud, ctx_rtu->config);
        _serial->receive();
#else
...
#ifdef ARDUINO
modbus_t* modbus_new_rtu(unsigned long baud, uint16_t config, void *serialType)
#else
